...
log_path = results_dir / f"eval_recourse_ensemble_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    members = []
    for seed in SEEDS:
        npy = results_dir / f"hgtm_ames_seed{seed}.npy"
        if not npy.exists():
    ...
    p = results_dir / f"eval_recourse_ensemble_{int(time.time())}.json"
